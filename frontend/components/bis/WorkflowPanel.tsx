...
function Limitations({ items }: { items: string[] }) { return items.length ? <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">{items.map((item) => <p key={item}>{item}</p>)}</div> : null; }
...
export default function WorkflowPanel({ workflow }: { workflow: Workflow }) {
  ...
  return <section aria-labelledby={`${workflow}-title`} className="surface-card rounded-2xl p-5 sm:p-7"><div className="h-1 w-10 rounded-full bg-[var(--bis-accent)]" /><h1 id={`${workflow}-title`} className="mt-4 font-display text-2xl font-semibold tracking-[-0.03em] text-[var(--bis-navy)]">{content.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--bis-ink-soft)]">{content.description}</p><label className="mt-6 block text-sm font-semibold text-[var(--bis-ink)]">{content.label}<textarea value={value} onChange={(event) => setValue(event.target.value)} rows={3} placeholder={content.placeholder} className="field-control mt-2 min-h-28 w-full resize-none rounded-xl px-4 py-3 text-sm leading-6" /></label>{workflow === "laboratories" && <label className="mt-4 block text-sm font-semibold text-[var(--bis-ink)]">Standard number <span className="font-normal text-[var(--bis-ink-soft)]">(optional)</span><input value={standardNumber} onChange={(event) => setStandardNumber(event.target.value)} placeholder="e.g. IS 302" className="field-control mt-2 w-full rounded-xl px-4 py-3 text-sm" /></label>}<button onClick={submit} disabled={loading} className="primary-button mt-5 min-h-12 rounded-xl px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{loading ? "Searching BIS evidence..." : content.button}</button>{error && <p role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</p>}{response && <Result workflow={workflow} response={response} />}</section>;
}
